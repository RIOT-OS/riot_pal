...
def main():
    ...
    try:
        phil = PhilipIf.from_autodetect(baud=9600)
    except Exception as ex:
        logging.debug("Failed to autodetect PhilipIf: " + str(ex))
    phil = PhilipIf.from_autodetect()
    phil2 = PhilipIf.copy_driver(phil)
    phil.execute_changes()
    phil2.execute_changes()
    phil.reset_mcu()
    cmds = phil.get_command_list()
    logging.debug("==========================================================")
    for cmd in cmds:
        cmd()
        logging.debug("------------------------------------------------------")
